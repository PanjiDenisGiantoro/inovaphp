<div class="row">
<div class="col-lg-12">
<h1 class="page-header">Laporan</h1>
</div>
<!-- /.col-lg-12 -->
</div>


<ul class="nav">
	<li><a href="?page=Laporan-Petugas">Laporan Data Petugas</a></li>
	<li><a href="?page=Laporan-Tindakan">Laporan Data Tindakan</a></li>
	<li><a href="?page=Laporan-Obat">Laporan Data Obat</a></li>
	<li><a href="?page=Laporan-Dokter">Laporan Data Dokter</a></li>
	<li><a href="?page=Laporan-Pasien">Laporan Data Pasien</a></li>
	<li><a href="?page=Laporan-Pendaftaran">Laporan Pendaftaran</a></li>
	<li><a href="?page=Laporan-Pendaftaran-Periode">Laporan Pendaftaran per Periode</a></li>
	
	<li><a href="?page=Laporan-Rawat">Laporan Rawat Pasien</a></li>
	<li><a href="?page=Laporan-Rawat-Periode">Laporan Rawat Pasien per Periode</a></li>
	<li><a href="?page=Laporan-Rawat-Pasien">Laporan Rawat Pasien per Pasien</a></li>
	
	<li><a href="?page=Laporan-Penerimaan-Obat">Laporan Penerimaan Obat </a></li>
	<li><a href="?page=Laporan-Penerimaan-Periode">Laporan Penerimaan Obat Pasien per Periode </a></li>
	
	<li><a href="?page=Laporan-Penjualan">Laporan Penjualan Obat</a></li>
	<li><a href="?page=Laporan-Penjualan-Periode">Laporan Penjualan Obat per Periode</a></li>
</ul>	